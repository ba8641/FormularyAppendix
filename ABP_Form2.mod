...
maximize TSSForm: 
WOri*(sum{b in F, m in T[b]} R[b,m]*X[b,m]- sum{f in F,b in f}C[b]*sum{m in T[b]}X[f,m]/(min{b in f}S[b]+1e-10)) - WSat*(sum{a in A, m in M: gni_p[m]<targetGNI}(d[a,m]*l[m] - sum{b in F1[a]}X[b,m]))- sum{b in B}g[b]; #Tweight-TS
...
subject to Capacity{b in B, f in F}: sum{m in M, f in F2[b]} X[f,m] <= S[b]*g[b];

subject to Capacity2{f in F, b in f}: sum{m in M} X[f,m] <= S[b]*g[b];

subject to Annuity2{f in F}: sum{m in M}Y[f,m]*X[f,m] >= sum{b in f} C[b]*g[b];
